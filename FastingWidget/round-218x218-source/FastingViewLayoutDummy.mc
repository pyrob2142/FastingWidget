...
class FastingViewLayoutDummy extends WatchUi.View {
    ...
    function onUpdate(dc) {
        ...
        switch (fast_manager.current_page) {
    ...
    function drawSummary(dc) {
    	...
    	var duration_label = "12H 23MIN 35S";
    	var calories_label = 15238.8.format("%.1f");
	...
	function drawFastWithoutGoal(dc) {
		...
		var time_label = "23H 15MIN 24S";
		var start_label = "06/04/20\n18:10";
	...
	function drawFastWithGoal(dc, show_remaining) {
		...
		if (show_remaining == true) {
			...
			time_label = "23H 15MIN 24S";
			date_label = "06/04/20, 18:10";
		}
		
		
		var progress = fast_manager.getProgress();
		var progress_label = "1002.3%";
	...
	function drawCalories(dc) {
		...
		dc.drawBitmap(center_x - 32, center_y - 90, resource_manager.bitmap_burn);
		dc.drawText(center_x, center_y + 15, Graphics.FONT_NUMBER_HOT, calories_label, Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
